...
|| empty($exp)||empty($achi)||empty($skill)||empty($partner)||empty($pwd)||empty($address2)||empty($img)||empty($pwd2))
{
  echo 4;
}
else {
 ...
 if (mysqli_num_rows($emailResult) > 0) {
   ...
   $pic= rand(100,500).".".$exe;
    ...
    VALUES ('$name', '$mail', '$dob', '$phone', '$desc', '$city', '$exp', '$achi', '$skill', '$partner', '$pwd', '$address2', '$pic', 'pending')";
   
   $sqll="INSERT INTO `login_users`(`login_name`, `login_mail`, `login_pass`,`login_role`) VALUES ('$name','$mail', '$pwd', 2)";
